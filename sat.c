 ...
 * occurred.
 */
SummedAreaTable *AllocateSat(uint32_t width, uint32_t height, uint16_t max) {
...
SummedAreaTable *PgmToSat(const PgmImage *pgm) {
    ...
    SummedAreaTable *sat = AllocateSat(width, height, pgm->max_gray_);
    if (!sat) {
        return NULL;
    }
...
uint64_t SatQuery(const SummedAreaTable *sat, uint32_t tlx, uint32_t tly,
    ...
    if (tlx > 0 && tly > 0)
        res += sat->data_[(tly - 1) * sat->width_ + tlx - 1];
    return res;
}
