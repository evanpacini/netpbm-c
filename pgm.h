 ...
 * Each pixel becomes the average of a square surrounding that pixel with side 2r + 1.
...
PgmImage *KasperBlur(PgmImage *image, int8_t radius) {
    ...
    // Blur pixel data
    #pragma omp parallel for default(none) shared(image, new_image, radius) collapse(2)
        ...
        for (uint32_t x = 0; x < image->width_; x++) {
            ...
            for (int64_t ry = -radius; ry <= radius; ry++) {
                for (int64_t rx = -radius; rx <= radius; rx++) {
                    if (y + ry >= 0 && y + ry < image->height_ && x + rx >= 0 && x + rx < image->width_) {
